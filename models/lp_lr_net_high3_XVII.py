...
class LPLRNet_XVII(nn.Module):
    def __init__(self, args,nrb_low=5, nrb_high=2, num_high=3):
        ...
        self.conv0_1 = nn.Conv2d(192,256,3,padding=1)
        
        self.mask_conv1 = nn.Conv2d(19, 3, 3, padding=1)
        self.mask_conv2 = nn.Conv2d(7, 3, 3, padding=1)
        self.mask_conv3 = nn.Conv2d(4, 3, 3, padding=1)
        self.conv1_1 =  nn.Conv2d(19,19,3,padding=1)
        self.conv1_2 = nn.Conv2d(19, 45, 1)
        self.resblock1_1 = modifiedResBlock(64)
        #self.calayer2 = CALayer(16)
        
        self.conv2_1 = nn.Conv2d(23,23,3,padding=1)
        #self.resblock2 = ResidualBlock()
        self.conv2_2 = nn.Conv2d(23, 9, 1)
        ...
        self.resblock3_1 = modifiedResBlock(16)
    ...
    def forward(self, real_A_full):
        ...
        y_fused_1 = self.conv0_1(y_fused)
        y_fused_1 = self.esa_1(y_fused_1)
        y_mask_1 = F.pixel_shuffle(y_fused_1, 4)
        y_mask_2 = F.pixel_shuffle(y_fused_1, 8)
        y_mask_3 = F.pixel_shuffle(y_fused_1, 16)
        mask3 = self.mask_conv1(torch.cat((y_mask_1,pyr_A[-2]), dim=1))
        mask2 = self.mask_conv2(torch.cat((y_mask_2,pyr_A[-3]), dim=1))
        mask1 = self.mask_conv3(torch.cat((y_mask_3,pyr_A[-4]), dim=1))
        pyr_new = []
        pyr_new.append(torch.mul(mask1, pyr_A[0]))
        pyr_new.append(torch.mul(mask2, pyr_A[1]))
        pyr_new.append(torch.mul(mask3, pyr_A[2]))
        pyr_new.append(fake_B_low)
        fake_B_up = self.lap_pyramid.pyramid_recons(pyr_new)
        ...
        high1_1 = self.conv1_1(high_with_low1)+high_with_low1
        high1_2 = self.conv1_2(high1_1)
        high1 = self.resblock1_1(torch.cat((high1_1, high1_2), dim=1))
        ...
        high2_1 = self.conv2_1(high_with_low2)+high_with_low2
        high2_2= self.conv2_2(high2_1)
        high2 = self.resblock2_1(torch.cat((high2_1,high2_2), dim=1))
...
#    from ptflops import get_model_complexity_info
    ...
    model = LPLRNet_XVII(nrb_low=5, nrb_high=3, num_high=3,args=args).to("cuda")    
