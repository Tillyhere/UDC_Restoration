...
class DilatedParllelResidualBlockB_d(nn.Module):
    '''
    This class defines the ESP block, which is based on the following principle
        Reduce ---> Split ---> Transform --> Merge
    '''
...
class ESPLRNet(nn.Module):
    def __init__(self, in_c=4, out_c=3, args=None):
        ...
        dmodule = DilatedParllelResidualBlockB_d
...
class LPLRNet_II(nn.Module):
    def __init__(self, args,nrb_low=5, nrb_high=2, num_high=3):
        super(LPLRNet_II, self).__init__()
...
#    from ptflops import get_model_complexity_info
    ...
    model = LPLRNet_II(nrb_low=5, nrb_high=3, num_high=3,args=args).to("cuda")    
