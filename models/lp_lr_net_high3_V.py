...
#channel 48, res+2DuESP_a+res, with lr_features
ex = Experiment("LPLRNet_O")
...
class DuESP(nn.Module):
     def __init__(self, nIn, nOut, add=True):
        super().__init__()
        dmodule = DilatedParllelResidualBlockB
        resblock = ResidualBlock
        #self.act = nn.PReLU(nOut)
        self.dmodule_a = dmodule(nIn,nOut)
        self.dmodule_b = dmodule(nIn,nOut)
        self.resblock = resblock(nIn)
        #self.dmodule2 = dmodule(nIn,nOut)
        #self.conv_1x1_exp = C(nOut, nOut, 1, 1)
    
     def forward(self,x,res):
        x_1 = self.resblock(x)
        x_1 = self.dmodule_a(x_1)
        x_1 = x_1+res
        x_2 = self.dmodule_b(x_1)
        return x_2,x_1
...
class ESPLRNet(nn.Module):
    def __init__(self, in_c=4, out_c=3, args=None):
        ...
        self.res2_a = du_esp(48,48)
        self.res2_b = du_esp(48,48)
        
        self.res3_a = res_block(48)
        self.res3_b = res_block(48)
    ...
    def forward(self, x):
        ...
        res = y2
        y,res = self.res2_a(y2,res)
        y3,res = self.res2_b(y,res)
...
class LPLRNet_V(nn.Module):
    def __init__(self, args,nrb_low=5, nrb_high=2, num_high=3):
        super(LPLRNet_V, self).__init__()
...
#    from ptflops import get_model_complexity_info
    ...
    model = LPLRNet_V(nrb_low=5, nrb_high=3, num_high=3,args=args).to("cuda")    
...
#    from torchstat import stat
    ...
    #model = OurLRNet(args=args).to(args.device)
    #model = SmoothDilatedResidualAtrousBlock(channel_num=48).to("cuda")
    model = LPLRNet_O(nrb_low=5, nrb_high=3, num_high=3,args=args).to("cuda")
