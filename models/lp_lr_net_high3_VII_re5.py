...
class DilatedParllelResidualBlockB_k(nn.Module):
    '''
    This class defines the ESP block, which is based on the following principle
        Reduce ---> Split ---> Transform --> Merge
    '''
    def __init__(self, nIn, nOut, add=True, shuffle=True):
        '''
        :param nIn: number of input channels
        :param nOut: number of output channels
        :param add: if true, add a residual connection through identity operation. You can use projection too as
    ...
    def forward(self, input):
        ...
        if self.shuffle:
            combine = channel_shuffle(combine,2)
        # if residual version
        if self.add:
            combine = input + combine
        output = self.air(combine)
...
class ESPLRNet(nn.Module):
    def __init__(self, in_c=4, out_c=3, args=None):
        ...
        dmodule = DilatedParllelResidualBlockB_k
...
class LPLRNet_VII_re5(nn.Module):
    def __init__(self, args,nrb_low=5, nrb_high=2, num_high=3):
        super(LPLRNet_VII_re5, self).__init__()
