...
class OLEDDataset(Dataset):
    ...
    def __init__(
        ...
        assert mode in ["train", "val", "val_mega","test"]
        self.mode = mode
        self.args = args
        ...
        elif self.mode =="val_mega":
            self.source_dir = args.val_mega_source_dir
            self.target_dir = args.val_mega_target_dir
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            if self.args.do_resize:
                h, w, c = source.shape
                rnd_h = random.randint(0, max(0, h - self.args.h_size))
                rnd_w = random.randint(0, max(0, w- self.args.w_size))
                source = source[rnd_h: rnd_h+ self.args.h_size, rnd_w:rnd_w+ self.args.w_size, :]
                target = target[rnd_h: rnd_h+ self.args.h_size, rnd_w:rnd_w+ self.args.w_size, :]
        ...
        if self.mode in ["train", "val", "val_mega"]:
            target = torch.tensor(target.copy()).float().permute(2, 0, 1)
            target = (target - 0.5) * 2
...
def get_dataloaders(args, is_local_rank_0: bool = True):
    ...
    val_mega_dataset = OLEDDataset(args, mode="val_mega", is_local_rank_0=is_local_rank_0)
    ...
    print(len(val_mega_dataset))
    if len(val_mega_dataset):
        if args.distdataparallel:
            val_mega_sampler = torch.utils.data.distributed.DistributedSampler(
                val_mega_dataset, num_replicas=dist.get_world_size(), shuffle=True
            )

            shuffle = False
        else:
            val_mega_sampler = None
            shuffle = True
    ...
    return Data(
        train_loader=train_loader, val_loader=val_loader, val_mega_loader=val_mega_loader, test_loader=test_loader
    )
